---
title: Adaptive box-polar navigation for graph items
type: proposal
status: concentric-box experiment implemented, 2026-07-23
---

# Adaptive box-polar navigation
...
stops. Their **box radius** (Chebyshev distance,
`max(abs(x - origin.x), abs(y - origin.y))`) and screen-space angle around the
fixed origin form loose square bands and angular bands. Box-radius tolerance is
the Cartesian grid's adaptive pixel tolerance; angular tolerance is separately
adapted and clamped to 8–35 degrees. The angular seam goes through the largest
empty gap so a coherent spoke is not split at 0/360 degrees. As in the
Cartesian strategy, ambiguous cells split locally until every spatially
distinct stop has a unique box-band/sector address. Using box radius rather
than Euclidean distance keeps navigation membership aligned with the rendered
concentric boxes, including near their corners.

## Movement

At the origin, a screen-direction key enters the innermost box band at the stop
nearest that bearing. Away from the origin, the four screen directions always
cover the four box-polar moves exactly once. Their roles rotate in the quadrant
...
Outward/inward moves preserve a **goal angle**; angular moves preserve a **goal
box radius**. Selection first looks for the exact goal-band intersection in
the destination band, then falls back to the nearest stop across a sparse gap.
This is the box-polar equivalent of re-acquiring a text editor's goal column.
...
A fixed origin makes one finite coordinate frame. Square bands form a line,
...
- Alternating translucent square bands and sectors, with the current
  box-band/sector/cell emphasized.
- Faint concentric boxes and radial boundaries. With cardinally distributed
  items, the angular boundaries are the diagonals that subdivide each box.
- A marked polar origin.
- Per-item radial/tangential membership crosshairs: radial-arm opacity encodes
  box-band cadence; tangential-arm opacity encodes sector cadence.
- A dashed goal ray when an outward/inward step is displaced from its goal
  angle, or a dashed goal box when an angular step is displaced from its goal
  radius.
...
- Whether all-stop construction produces too many boxes/spokes on large
