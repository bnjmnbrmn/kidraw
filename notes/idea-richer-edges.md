...
- **Self-linking edges — shipped 2026-08-07; richer editing 2026-08-11.** Hold Add over the node, then
  `s` **Edge...** → `l` **Self Loop** (`a → s → l` in Vim; `e → s → l`
  in IJKL). Every loop starts with two draggable waypoint handles; repeated
  creation on one node produces distinct nested loops, and both the routes
  and waypoint identities survive snapshot/file round trips.
