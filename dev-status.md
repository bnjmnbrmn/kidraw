...
The final step of the interaction-model roadmap (edge labels ✅ → move-by-graph ✅ 2026-07-13, see Current focus item 13 → **gathering/collapse**). Design input: [`notes/idea-gather-recursive.md`](notes/idea-gather-recursive.md) — should be recursive and push away nodes; relates to applying layouts more generally. Note for the design: the traversal direction metric (`endpointFlowDirection` in `graph-nav.ts`) is deliberately swappable — jumping toward a gathered/sucked-in node may want bearing-to-endpoint instead of the departure tangent (the held-open part of traversal decision Q1). Possible traversal refinement from the same decision log: seed weak momentum from the last free crosshairs movement on a cold start.
