...
**Entry.** Opening "Move by graph…" (hold `f`, since the 2026-07-13 binding reorg) with a **single** node, edge, or label selected: move the crosshairs to that item and recenter the view on the crosshairs (skip both if the crosshairs are already there), in addition to bringing up the submenu as usual.
...
- Grid tier: `GridTier` (`fine`/`normal`/`coarse`) already flows through move/drag commands; the traversal commands need to carry it too. **Open design question:** how the tier is expressed while the `f` submenu is held (the reorg put move-by-graph on `f` partly because `f`+`s`/`f`+`d` are comfortable same-hand chords) — the coarse/fine speed keys are themselves held-key submenus, and multi-held-key chords are the known-buggy area of the keymenu state machine (see "Key-handling bugs" above). May be worth fixing bug B there first, or defining tier as a sticky mode inside the submenu.
...
- **Q3 — tier while `f` is held: still open** (interacts with the keymenu held-chord bugs; see "Key-handling bugs" above).
